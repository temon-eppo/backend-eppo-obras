...
// GET: descrições únicas
app.get('/api/ferramentas/descriptions', async (req, res) => {
  try {
    const descriptions = await getUniqueDescriptions();
    res.json(descriptions);
  } catch (err) {
    console.error(err);
    res.status(500).send('Erro ao buscar descrições únicas');
  }
});
...
// GET: todos os funcionários ou filtrado por obra
app.get('/api/employees', async (req, res) => {
  try {
    const obra = req.query.obra;
    let employees;
    if (obra) {
      employees = await getEmployeesByObra(obra);
    } else {
...
app.get('/api/employees/name/:name', async (req, res) => {
  try {
    const employees = await getAllEmployees(); // ou crie getEmployeeByName no db.js
    const filtered = employees.filter(f => f.NOME.toLowerCase() === req.params.name.toLowerCase());
    if (!filtered.length) return res.status(404).send('Funcionário não encontrado');
    res.json(filtered);
  } catch (err) {
    console.error(err);
